...
public class InvoiceBilling {
    ...
    public static void viewAllInvoices() {
        try (Connection connection = DataBaseConnection.getConnection()) {
            String query = "SELECT i.InvoiceID, c.Name, i.Date, i.Total FROM Invoices i JOIN Customers c ON i.CustomerID = c.CustomerID";
            try (PreparedStatement stmt = connection.prepareStatement(query)) {
                ResultSet rs = stmt.executeQuery();
                System.out.printf("%-10s %-20s %-10s %-10s\n", "InvoiceID", "Customer Name", "Date", "Total");
                while (rs.next()) {
                    System.out.printf("%-10d %-20s %-10s %-10.2f\n", rs.getInt("InvoiceID"), rs.getString("Name"), rs.getDate("Date"), rs.getDouble("Total"));
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public static void viewInvoiceByID(int invoiceId) {
        try (Connection connection = DataBaseConnection.getConnection()) {
            ...
            String query2 = "SELECT i.SubTotal, i.Discount, i.Total FROM Invoices i JOIN Customers c ON i.CustomerID = c.CustomerID WHERE i.InvoiceID = ?";
            try (PreparedStatement stmt = connection.prepareStatement(query2)) {
                stmt.setInt(1, invoiceId);
                ResultSet rs = stmt.executeQuery();
                if (rs.next()) {
                    System.out.printf("SubTotal: %.2f\nDiscount: %.2f\nTotal: %.2f\n",
                            rs.getDouble("SubTotal"), rs.getDouble("Discount"), rs.getDouble("Total"));
                } 
            }
        } catch (SQLException e) {
            e.printStackTrace();
